...
public class AuthController {
  ...
  public String showLoginForm(Model model) {
    if (!authService.isAnonymous()) { // usar helper explícito
   ...
   *
   * Caso permitido:
   *   - Usuario anónimo (alta pública), o
   *   - Usuario logado con rol administrador (alta manual).
   *
   * Si un usuario logado NO es admin, se redirige a /main.
   *
  ...
  public String showRegisterForm(Model model) {
    if (!authService.isAnonymousOrAdmin()) { // helper semántico
      return "redirect:/main";
    }
   ...
   * 3. En función del contexto:
   *    - Si es registro público → inicia sesión automáticamente.
   *    - Si es alta de admin → crea el usuario sin afectar a la sesión.
   *
   * ----------------------------------------------------------------------------
   ...
   *     → Alta de usuario realizada por un administrador, sin iniciar sesión
   *       con la nueva cuenta.
   *
  ...
  public String processRegister(
    ...
    // Registro público → iniciar sesión con la nueva cuenta
    ...
    // Alta realizada por un admin → mantener la sesión actual
    return "redirect:/userlist/list";
  }
 ...
 *     }
 *
 * Modificación para /register:
 *     - Se permite también a administradores (alta manual):
 *       authService.isAnonymousOrAdmin()
 *
 * ----------------------------------------------------------------------------
